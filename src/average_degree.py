...
def calculateAvg(mapping):
	...
	outputFile.write("\n")
        ...
        outputFile.write(("%.3f" % (sum*1.0/counter))[:-1])
	outputFile.write("\n")
def retriveHashtag(tags):
        ...
        if len(tags)==0:
                ...
                if len(tempList)==2:
			## empty hashtag list, ignore
                        return hashtagMap
                else:
			## get all hashtags and create list
                     	for tempItem in tempList:
                                if tempItem != "":
                                        tempHashtag = tempItem.split('","indices"')[0]
                                        hashtagMap[tempHashtag]=0
                        if len(hashtagMap) == 1:
			    ## only 1 distinct hashtag, ignore
                            return {}
                        else:
			    ## remove already existing links from new hashtags and update list
                                ...
                                for k in tempKeys[position+1:]:
                                    ...
                                    if len(mainMap) > 0:
                                        for m in mainMap.keys():
                                            if ((j not in mainMap[m].keys()) or (k not in mainMap[m].keys())):
                                                counter += 1
                            ...
                            for n in hashtagMap.keys():
                                if hashtagMap[n] == 0:
                                    del hashtagMap[n]
                            return hashtagMap
def updateMap(hashtagList,maxTime):
	#function that update mainMap list given the hashtag list and maxTime
	global mainMap
